...
class PhasorGroup:
    ...
    def __init__(self,complex_array=np.array([])):
        ...
        self.angle = np.angle(complex_array)%(2*np.pi)
    ...
    def plot(self,ax,pad=1.0,markup_color=[0.8,0.8,0.8]):
        ...
        for ang in np.arange(0,2,0.25):
            ...
            plt.text(x,y,'%0.2f $\pi$'%ang,ha=ha,va=va,color=markup_color)
        ...
        hl = 0.1
        for angle,magnitude in zip(self.angle.ravel(),self.magnitude.ravel()):
            ...
            plt.text(px,py,'(%0.1f,%0.1f)'%(px,py),color=color)
